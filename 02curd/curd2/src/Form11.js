    ...
    return(
        <div className="form-overlay">
            <form>
                ...
                <div className="form-group">
                    <label>Price:</label>
                    <input className="form-control mt-2" value={product.price} 
                     onChange={changeFormData} type="number" name="price" placeholder="Enter Product Price"
                    ></input>
                    {
                        // sumitted && product.price ==="" && <span className="text-danger">Product price required</span>
                        sumitted && (!product.price || isNaN(product.price) || Number(product.price) <= 0) &&
                        <span className="text-danger">Valid product price required (must be greater than 0)</span>
                    }
                </div>
                <div className="form-group">
